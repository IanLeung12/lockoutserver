...
# =============================================================================
# Minecraft 26.2 Fabric Server Startup Script
# =============================================================================
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  Minecraft 26.2 Fabric Server${NC}"
echo -e "${GREEN}========================================${NC}"

# Check Java
if ! command -v java &> /dev/null; then
    echo -e "${RED}[ERROR] Java is not installed. Run: apt install temurin-25-jdk -y (see userdata.sh for the Adoptium repo)${NC}"
    exit 1
fi

JAVA_VERSION=$(java -version 2>&1 | awk -F '"' '/version/ {print $2}' | cut -d'.' -f1)
if [ "$JAVA_VERSION" -lt 25 ]; then
    echo -e "${RED}[ERROR] Java 25+ is required. Found Java ${JAVA_VERSION}.${NC}"
    exit 1
fi
